    ...
    <div class="hidden lg:flex flex-col justify-between p-12" style="width: 420px; background: linear-gradient(160deg, #0c4a0d 0%, #1a6b1c 55%, #246b24 100%); flex-shrink: 0;">
        ...
        <div>
            ...
            <div class="mt-8 space-y-3">
                @foreach([['☕', 'Supply Chain', 'Pengadaan biji kopi langsung dari petani'], ['📊', 'Dashboard CRM', 'Monitor operasional café secara real-time'], ['🔔', 'Broadcast WA', 'Notifikasi otomatis ke seluruh jaringan petani']] as [$icon, $title, $desc])
                <div class="flex items-start gap-3">
                    <span class="text-2xl">{{ $icon }}</span>
                    <div>
                        <p style="color:#fff;font-weight:600;font-size:0.85rem;">{{ $title }}</p>
                        <p style="color:rgba(255,255,255,0.5);font-size:0.78rem;">{{ $desc }}</p>
                    </div>
                </div>
                @endforeach
            </div>
